...
skel_data = cleanUp(raw_data);

unique_timestamps = unique(skel_data(:,log_c_tracking_time), 'rows');
unique_timestamps_count = size(unique_timestamps, 1);
unique_persons = unique(skel_data(:,log_c_person), 'rows');
...
for row_idx = 1:numel(unique_timestamps)
    ...
    selected_t = skel_data(skel_data(:,log_c_tracking_time)==timestamp,:);
...
for row_idx = 1:numel(unique_persons)
    ...
    for joint_idx = 1:joint_count
        ...
        % Hack: Assume 2 skeletons             
        dx_avg = mean(selected_j(:,1));
        dx_sd = std(selected_j(:,1));
        dy_avg = mean(selected_j(:,2));
        dy_sd = std(selected_j(:,2));
        dz_avg = mean(selected_j(:,3));
        dz_sd = std(selected_j(:,3));
        dd_avg = mean(selected_j(:,4));
        dd_sd = std(selected_j(:,4));
        
        avg_data_idx = avg_c_joint+(joint_idx-1)*avg_data_per_joint;
        avg_at_p(avg_data_idx:avg_data_idx+avg_data_per_joint-1) = [dx_avg dx_sd dy_avg dy_sd dz_avg dz_sd dd_avg dd_sd];
    end
...
% Hack: For 1 person
% 
average_joint_table = zeros(unique_timestamps_count*unique_persons_count,average_c_count);
...
for joint_idx = 1:joint_count
    ...
    all_joints_table(joint_idx,4) = avg_data(1,avg_joint_idx+1);
    % dy
    all_joints_table(joint_idx,5) = avg_data(1,avg_joint_idx+2);
    all_joints_table(joint_idx,6) = avg_data(1,avg_joint_idx+3);
    % dz
    all_joints_table(joint_idx,7) = avg_data(1,avg_joint_idx+4);
    all_joints_table(joint_idx,8) = avg_data(1,avg_joint_idx+5);
    % dd
    all_joints_table(joint_idx,9) = avg_data(1,avg_joint_idx+6);
    all_joints_table(joint_idx,10) = avg_data(1,avg_joint_idx+7);
end
...
time_independent_dd_avg = mean(all_joints_table(:,all_c_dd_avg));
time_independent_dd_sd = std(all_joints_table(:,all_c_dd_avg));
all_scenarios_table(1,3:4) = [time_independent_dd_avg time_independent_dd_sd];
% plot
